...
socket.on("live_counts", (data) => updateLiveCounts(data));
...
socket.on("answer_revealed", (data) => {
  if (currentState) {
    ...
    currentState.reveal_data = data;
    updateQuestion(currentState.question, currentState.current_question_index,
                   currentState.total_questions, true);
...
socket.on("answer_hidden", () => {
  if (currentState) {
    ...
    updateQuestion(currentState.question, currentState.current_question_index,
                   currentState.total_questions, false);
    hideResults();
...
function applyState(state) {
  ...
  updateButtons(state);
  updateQuestion(state.question, state.current_question_index,
                 state.total_questions, state.answer_revealed);
  updateCounters(state.students_online);
  if (state.live_counts) updateLiveCounts(state.live_counts);
...
function updateButtons(state) {
  ...
  revealBtn.classList.toggle("hidden", !!state.answer_revealed);
  hideBtn.classList.toggle("hidden", !state.answer_revealed);

  prevBtn.disabled = state.current_question_index === 0;
  nextBtn.disabled = state.current_question_index >= (state.total_questions - 1);
}

function updateQuestion(q, idx, total, showCorrect = false) {
  if (!q) {
...
// ── Results (shown only after reveal) ────────────────
function hideResults() {
  resultsPanel.classList.add("hidden");
  resultsPanel.closest(".question-panel").classList.remove("revealed");
}
  ...
  if (data.question_type === "multiple_choice" || data.counts !== undefined) {
    ...
    if (responses.length === 0) {
      ...
      responses.forEach((r) => {
        ...
        chip.innerHTML = (r && typeof r === "object")
          ? `<span>${escapeHtml(r.text)}</span>${r.count > 1 ? `<span class="chip-count">${r.count}</span>` : ""}`
          : escapeHtml(r);
